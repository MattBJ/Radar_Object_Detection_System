...
def generate_y_data(y_0, v_0, nf_pos, nf_vel): # count is in milliseconds
...
y_data, count, actual_y, y_vel_data, actual_y_vel = generate_y_data(y_0 = y_0, v_0 = dy_0, nf_pos = nf_pos, nf_vel = nf_vel)

x_0 = 0
dx_0 = 2 # .3 meters/s

x_data = np.array([np.random.randn() * nf_pos + x_0 + (dx_0)*(i/1000) for i in range(count)])

actual_x = np.array([x_0 + (dx_0)*(i/1000) for i in range(count)])

x_vel_data = np.array([np.random.randn()*nf_vel + dx_0 for i in range(count)])
...
R = np.diag([.75,10000,.75,10000]) # x, y measurement noise covariance
...
# FINAL NOTES FOR THIS IMPLEMENTATION:
# Q matrix is still larger than I was hoping? Still need to read the Full math implementation, but maybe it's correct

print(x[489],y[489])
print(track[489,0],track[489,1])

print("residual of final coordinates: ", track[489,0]-x[489],track[489,1]-y[489])